		...
		try:
			...
			if(match_result is not None and len(match_result)):
				for i, ipstring in enumerate(match_result):
					# note: findall always return a list if there is a match
					serverIp = re.findall(ip_address_pattern, ipstring.text)
				...
				for j in range(len(iplist)):
					...
					if(len(iplist) > 1):
						...
						index_str = f"{index+1}"
					print(
						f'{index_str:<6}:  {iplist[j]:<15}    {servername:<{width}}  Latency : {latency}ms')
...
def GetIPAddressWithXpath(ServerList=None):
	...
	for index, host in enumerate(ServerList):
		try:
			...
			ip_list = elements_selector.xpath(
				'/html/body/div/main/section/table/tbody/tr/td/ul[@class="comma-separated"]/li/text()')
				...
				for j, ipstring in enumerate(ip_list):
					...
					if serverIp is not None:
						...
						if(len(ip_list) > 1):
							...
							index_str = f"{index+1}"
						print(
							f'{index_str:<6}:  {serverIp:<15}    {host:<{width}}  Latency : {latency}ms')
			else:
...
def updateHostsConfigFile(newEntriesList):
	...
	for entry in newEntriesList:
		new_entry = HostsEntry(
			entry_type='ipv4', address=entry['Ip'], names=[entry['Host']])
